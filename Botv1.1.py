            ...
            try:
                ...
                if current_date >= end_date_date:
                    ...
                    try:
                        await context.bot.send_message(
                            chat_id=user_id,
                            text=f"➕ Начислено {daily_profit:.6f} PULS\n"
                                 f"Текущий доход: {new_earned:.6f} PULS\n"
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'balance':
            ...
            for stake in staking_sheet.get_all_records():
                if str(stake.get('user_id')) == str(user_id) and stake.get('status') == 'Активен':
                    ...
                    active_stakes.append(
                        f"• {stake['amount']} PULS ({stake['period']} мес.) → {stake['daily_profit']:.4f} PULS/день"
                    )
        ...
        if context.user_data['currency'] == 'USDT':
            ...
            await context.bot.send_message(chat_id=query.from_user.id, text=f"Текущий курс PULS: 1 USDT = {rate:.2f} PULS Введите сумму пополнения: ")
        else:
            await query.message.delete()
...
async def send_presentation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.info(f"Запрос на презентацию от {query.from_user.id}")

    try:
        # Абсолютный путь к файлу
        current_dir = os.path.dirname(os.path.abspath(__file__))
        presentation_path = os.path.join(current_dir, "assets", "pulsar_presentation.pdf")

        # Проверка существования файла
        if not os.path.exists(presentation_path):
            logging.error(f"Файл не найден: {presentation_path}")
            await query.message.reply_text("❌ Файл презентации не найден на сервере")
            return

        logging.info(f"Файл найден, размер: {os.path.getsize(presentation_path)} байт")
        ...
        # Отправка файла с таймаутом
        try:
            with open(presentation_path, 'rb') as file:
                await context.bot.send_document(
                    chat_id=query.message.chat_id,
                    document=file,
                    filename="PulsarFinance_Presentation.pdf",
                    caption="📄 Официальная презентация Pulsar Finance",
                    read_timeout=30,
                    write_timeout=30,
                    connect_timeout=30
                )
            logging.info("Презентация успешно отправлена")

        except telegram.error.TimedOut:
            logging.error("Таймаут при отправке файла")
            await query.message.reply_text("⌛ Не удалось отправить файл: превышено время ожидания")
