...
import os  # Добавьте, если ещё нет
import pytz
...
creds = Credentials.from_service_account_file("pulsar-466316-bea388d0119c.json", scopes=scope)
...
def append_transaction(sheet, transaction_data: dict):
    ...
    try:
        ...
        row_data = [
            ...
            transaction_data.get('currency', 'PULS'),
            ...
            try:
                ...
                if current_date >= end_date_date:
                    ...
                    try:
                        await context.bot.send_message(
                            ...
                            text=f"🎉 Стейкинг завершен!\n"
                                 f"• Возвращено: {amount} PULS\n"
                                 f"• Начислено: {earned:.2f} PULS\n"
                                 f"• Новый баланс: {new_balance} PULS"
                        )
                    except Exception as e:
                    ...
                    try:
                        await context.bot.send_message(
                            chat_id=user_id,
                            text=f"➕ Начислено {daily_profit:.2f} PULS\n"
                                 f"Текущий доход: {new_earned:.2f} PULS\n"
                                 f"Баланс: {round(get_balance(user_id), 2)} PULS"
                        )
                    except Exception as e:
                        logging.error(f"Не удалось отправить уведомление пользователю {user_id}: {e}")
...
async def input_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_address"):
        if not text:
            await update.message.reply_text("Пожалуйста, укажите адрес в сети BEP-20 для вывода средств")
            return INPUT_AMOUNT
        ...
        try:
            ...
            await update.message.reply_text(
                f"✅ Заявка на вывод {amount} PULS отправлена\n"
                f"📭 Адрес: {address}",
    ...
    if 'stake_period' in context.user_data:
        ...
        try:
            ...
            daily_profit = round(year_profit / 365, 2)
            ...
            await update.message.reply_text(
                ...
                f"Сумма: {amount} PULS\n"
                f"Годовой %: {percent}%\n"
                f"Доход в день: {daily_profit} PULS\n"
                f"Общий доход: {total_profit} PULS\n\n"
    ...
    try:
        amount = float(text)
        currency = context.user_data.get('currency', 'PULS')
        adress_usdt = 0x700d89999E9F8E47e860520aBB57DBF774081bAd
        adress_puls = 'Coming soon'
        if currency == 'USDT':
            puls_rate = get_puls_rate()
            converted = round(amount / puls_rate, 2)
            adress = adress_usdt
        else:
            converted = amount
            adress = adress_puls
        ...
        await update.message.reply_text(f"`{adress}`", parse_mode='Markdown')
        await update.message.reply_text(
            f"Вы выбрали пополнение в {currency} на сумму {amount}. \n"
            f"Это будет зачтено как ~{converted} PULS. \n"
            f"Пожалуйста, отправьте скриншот/фото подтверждения оплаты."
            f"\n\n*Используйте сеть исключительно BEP-20*", parse_mode='Markdown'
        )
...
def save_transaction(user_id, username, currency, amount, timestamp, status, photo_url, raw_amount=None):
    ...
    sheet.append_row([
        user_id, balance, "PULS", round(amount, 2), timestamp, status, photo_url, username
    ])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await message.reply_text("Добро пожаловать в проект Pulsar!\n🚀 Присоединяйся сейчас и начни зарабатывать с первых минут!", reply_markup=reply_markup)
    ...
    if query.data == 'balance':
        try:
            ...
            for stake in staking_sheet.get_all_records():
                ...
                if (str(stake.get('user_id')) == str(user_id) and
                        stake.get('status', '').lower().strip() in {'активен', 'ожидает старта'}):

                    frozen += float(stake['amount'])
                    active_stakes.append(
                        f"• {stake['amount']} PULS ({stake['period']} мес.) → {stake['daily_profit']:.2f} PULS/день"
                    )
            ...
            message = (
                f"💰 Текущий баланс: **{balance:.2f} PULS**\n"
                f"❄️ Заморожено в стейкинге: **{frozen:.2f} PULS**\n\n"
            )
        ...
        keyboard = [
            [InlineKeyboardButton("USDT", callback_data='usdt'), InlineKeyboardButton("PULS", callback_data='puls')],
            [InlineKeyboardButton("🔙 Назад", callback_data='home')]]
        await query.edit_message_text(text=f"Текущий курс PULS: 1 PULS = {puls_rate:.2f} USDT \nВыберите валюту для пополнения:", reply_markup=InlineKeyboardMarkup(keyboard))

    elif query.data in ['usdt', 'puls']:

        context.user_data['currency'] = query.data.upper()
        if context.user_data['currency'] == 'USDT':
            rate = get_puls_rate()
            context.user_data['puls_rate'] = rate
            await query.message.delete()
            await context.bot.send_message(chat_id=query.from_user.id, text=f"Текущий курс PULS: 1 PULS = {rate:.2f} USDT Введите сумму пополнения: ")
        else:
            await query.message.delete()
        ...
        if balance < 1:
            ...
            await query.message.delete()
            await context.bot.send_message(chat_id=user_id, text="Введите сумму для вывода в PULS:")
        ...
        await query.edit_message_text("📥 Стейкинг\n\nЗастейкай монеты PULS и получай пассивный доход каждый день.\nСредства замораживаются на выбранный срок, а прибыль начисляется автоматически по фиксированной ставке.\n\nДоступные планы:\n▪️ 1 месяц — 60% годовых\n▪️ 3 месяца — 80% годовых\n▪️ 6 месяцев — 120% годовых\n▪️ 12 месяцев — 180% годовых\n\n📈 Начни зарабатывать уже сегодня — выбирай срок и сумму стейкинга ниже!", reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        try:
            ...
            user_stakes = [
                ...
                if str(stake.get('user_id', '')) == str(user_id)
                   and stake.get('status', '').lower().strip() in {'активен', 'ожидает старта'}#and stake.get('status', '').lower() == 'активен'
            ]
                ...
                try:
                    ...
                    if start_date_str:
                        try:
                            ...
                            date_info.append(f"⌛ *Начало:* {start_date.strftime('%d.%m.%Y')}")
                        except ValueError:
                            date_info.append("⌛ *Начало:* неверный формат")

                    if end_date_str:
                        try:
                            end_date = datetime.strptime(end_date_str, "%Y-%m-%d %H:%M:%S")
                            date_info.append(f"⏳ *Окончание:* {end_date.strftime('%d.%m.%Y')}")
                            days_left = (end_date - datetime.now()).days
                            if days_left > 0:
                                date_info.append(f"📅 *Осталось дней:* {days_left}")
                        except ValueError:
                            date_info.append("⏳ *Окончание:* неверный формат")

                    date_info_str = "\n".join(date_info) if date_info else "📅 *Даты:* не указаны"
            ...
            if user_stakes:
                message_lines.append("\n━━━━━━━━━━━━━━━━━━━━\n✨ *Используйте кнопки ниже для управления стейкингом*")
        ...
        keyboard = [
            [InlineKeyboardButton("📩 Скачать презентацию", url='https://t.me/c/2224105724/20')],  #callback_data='get_file'
            [InlineKeyboardButton("🌐 Наша Группа", url='https://t.me/+XOGqCwsOrK80NDM0')],
            [InlineKeyboardButton("🔙 Назад", callback_data='home')]
        ]
        await query.answer()  # Важно: сразу отвечаем на запрос
        await query.edit_message_text("🏢Pulsar Finance — это инновационная DeFi-платформа на Binance Smart Chain, созданная для тех, кто хочет получать пассивный доход безопасно и просто. Наш проект объединяет передовые технологии блокчейна и удобный интерфейс Telegram-бота, чтобы каждый мог зарабатывать на криптовалюте без сложных инструментов.\n\n👥 Команда Pulsar\nМы — группа энтузиастов крипторынка с опытом в DeFi, блокчейн-разработке и маркетинге. Наша цель — создать продукт, который принесет пользу каждому инвестору, от новичков до профессионалов. \n\n🔹 Основные функции:\n✅ Стейкинг $PULS – вкладывайте токены и получайте до 120% годовых\n\n✅ Гибкие сроки – от 1 до 12 месяцев с разной доходностью\n\n✅ Автоматические выплаты – ежедневное начисление процентов\n\n✅ Реферальная программа – до 5% с депозитов приглашенных\n\n✅ Простое управление – удобный интерфейс и мгновенные транзакции", reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        await show_referral_stats(update, context)
            ...
            await query.edit_message_text("🔗 Полезные ссылки проекта Pulsar\n\n🌐 Официальный сайт:\nhttps://pulsar-project.io\n\n📢 Telegram-канал:\nhttps://t.me/+0Hg25fR1yeMyYTg6\n\n💬 Telegram-группа:\nhttps://t.me/+XOGqCwsOrK80NDM0\n\n📈 График монеты:\nDexTools (будет позже)\n\nCoinMarketCap (после листинга)\n\n📘 Whitepaper:\nОткрыть документ (PDF)\n\n🧩 NFT-коллекция (в разработке):\nСмотреть на OpenSea\n\n✉️ Поддержка:\nhttps://t.me/CryptusMenAdmin", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def send_file_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    file_path = r"C:\Users\saran\PycharmProjects\PythonProject\assets\pulsar_presentation.pdf"
    if os.path.exists(file_path):
        await context.bot.send_document(chat_id=query.message.chat.id, document=InputFile(file_path))
    else:
        await context.bot.send_message(chat_id=query.message.chat.id, text="⚠️ Файл не найден.")
...
async def show_referral_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Получаем базовую статистику рефералов
        try:
            referral_sheet = client.open("Bot").worksheet("Referrals")
        ...
        msg = (
            ...
            "💸 *Система бонусов:*\n"
            "• 1 уровень: 5% от пула реферала\n"
            "• 2 уровень: 2% от пула реферала\n"
            "• 3 уровень: 1% от пула реферала\n\n"
        ...
        error_keyboard = [[InlineKeyboardButton("🏠 На главную", callback_data='home')]]
        if hasattr(update, 'callback_query'):
            await update.callback_query.edit_message_text(
...
async def show_referral_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Получаем данные статистики напрямую (заменяем get_fresh_referral_stats)
        referral_sheet = client.open("Bot").worksheet("Referrals")
        bonus_sheet = client.open("Bot").worksheet("ReferralBonuses")

        # Ваша существующая логика сбора статистики
        stats = {
            'total': 0,
        ...
        stats['by_level'][1]['users'] = [r['username'] for r in level1 if 'username' in r][:5]
        ...
        msg += f"💰 Всего заработано: *{round(stats['total_bonus'], 2)} PULS*\n\n"
        ...
        for level in [1, 2, 3]:
            ...
            msg += (
                ...
                f"• Заработано: {round(stats['by_level'][level]['bonus'], 2)} PULS\n"
            )
        ...
        # Клавиатура без кнопки "Обновить"
        keyboard = [
        ...
        await message.reply_text("⚠️ Произошла ошибка при загрузке статистики")
...
async def daily_staking_task():
    while True:
        ...
        for i, row in enumerate(records, start=2):
            ...
            if datetime.now() >= end_date:
                ...
                user_id = row["user_id"]
                username = row.get("username", "")
                sheet.append_row([user_id, round(get_balance(user_id) + profit, 2), "PULS", round(profit, 2), datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Начисление дохода","",username])
                continue
    ...
    # обновление баланса в Google Sheets
        ...
        try:
            await context.bot.send_message(chat_id=user_id, text=f"Вам начислено {daily_profit} PULS за стейкинг.")
        except:
                logging.warning(f"Не удалось отправить сообщение пользователю {user_id}")
load_dotenv()
    ...
    try:
        ...
        if jq:
            jq.run_daily(
                ...
                time=time(hour=17, minute=00, tzinfo=pytz.timezone("Europe/Kyiv")),
        ...
        #app.add_handler(CallbackQueryHandler(send_presentation, pattern='^get_presentation$'))
        app.add_handler(CallbackQueryHandler(send_file_callback, pattern='^get_file$'))
